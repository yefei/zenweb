...
let failDefaultCode = -1;
let failDefaultHttpCode = 422;
...
function fail(msg) {
  ...
  const err = createError(httpCode || failDefaultHttpCode || 422, message);
  err[FAIL_BODY] = {
    code: code || failDefaultCode,
    message,
    data,
  };
  throw err;
 ...
 * 设置 fail 函数的默认状态码
 */
function setFailDefaultCode({ code, httpCode }) {
  if (code) failDefaultCode = code;
  if (httpCode) failDefaultHttpCode = httpCode;
}
