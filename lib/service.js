/**
 * @typedef {import('koa').Context} Context
 * @typedef {import('./core')} Core
 */
'use strict';

const debug = require('debug')('zenweb:service');

// Stack trace format :
// https://github.com/v8/v8/wiki/Stack%20Trace%20API
const stackReg = /at\s+(.*)\s+\((.*):(\d*):(\d*)\)/i;
const stackReg2 = /at\s+()(.*):(\d*):(\d*)/i;

class Service {
  /**
   * @param {Context} ctx 
   */
  constructor(ctx) {
    this.ctx = ctx;
  }
}

const registeredServices = new Map();

/**
 * 取得 services 对象，这里并不会取得所有 service, 而是在单独取得时候才会被初始化
 * @param {Context} ctx
 */
function getServicesByContext(ctx) {
  return new Proxy({}, {
    get(target, property) {
      if (!(property in target)) {
        if (!registeredServices.has(property)) {
          throw new Error(`Does not exist service: ${property}`);
        }
        debug('init: %s', property);
        const cls = registeredServices.get(property);
        target[property] = new cls(ctx);
      }
      return target[property];
    },
  });
}

/**
 * 注册 service
 * @param {<T extends Service>() => { new(ctx: Context): T }} cls service class
 * @param {string} [name] 自定义实例名称，如果不指定则自动使用 class name 首字母小写
 */
function registerService(cls, name) {
  if (!name) {
    name = cls.name.charAt(0).toLowerCase() + cls.name.slice(1);
  }
  if (registeredServices.has(name)) {
    throw new Error(`Duplicate service name: ${cls.toString()} => ${name}`);
  }
  if (debug.enabled) {
    // 取得调用位置
    const stack = new Error().stack.split('\n')[2];
    const sp = stackReg.exec(stack) || stackReg2.exec(stack);
    let path = '-';
    if (sp && sp.length === 5) {
      path = sp.slice(2).join(':');
    }
    debug('register: (%s) => %s', path, name);
  }
  return registeredServices.set(name, cls);
}

/**
 * 安装 service 服务
 * @param {Core} core
 */
function setup(core) {
  core.defineContextCacheProperty('service', getServicesByContext);
}

module.exports = {
  Service,
  setup,
  registerService,
};
